...
import onmt.modules
from onmt.gleu import sentence_gleu
import argparse
import torch
...
def sample(model, batch, dicts, eval=False):
		...
		batch_size = batch[0][0].size(1)
		
		indices = random.sample(range(batch_size), min(10, batch_size))
		
		for idx in indices:
				
			tgtIds = [v.data[0][idx] for v in sampled_sequence]
			
			tgtWords = tgtDict.convertToLabels(tgtIds, onmt.Constants.EOS)
			...
			ref = batch[1][1:]
			for i in xrange(ref.size(0)):
				
				refWords.append(ref[i])
				
			refIds = [v.data[idx] for v in refWords]
			
			refSentWords = tgtDict.convertToLabels(refIds, onmt.Constants.EOS)
			
			refSent = " ".join(refSentWords)
			
			print "   REF :", refSent
			
			gleu = sentence_gleu(refSentWords, tgtWords)
...
def trainModel(model, trainData, validData, dataset, optim):
    ...
    def trainEpoch(epoch, batchOrder=None):
        ...
        for i in range(nSamples):
            ...
            model.zero_grad()
            loss, _, _ = model(batch, eval=False)
            # Exclude <s> from targets.
            targets = batch[1][1:]
...
def main():
    ...
    model = onmt.Models.NMTModel(encoder, decoder, generator, criterion)
