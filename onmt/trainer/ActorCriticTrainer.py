...
class A2CTrainer(object):
    ...
    def run(self, checkpoint=None):
        ...
        def trainEpoch(epoch, batchOrder=None):
            ...
            total_critic_loss, report_critic_loss = dict(), dict()
            report_rewards, report_tgt_words = dict(), []
            report_tgt_sents, total_tgt_words = dict(), dict()
            report_src_words = []
            start = time.time()
            model.zero_grad()
            ...
            for i in validSets:
                ...
                total_critic_loss[i] = 0
                report_critic_loss[i] = 0
                total_tgt_words[i] = 0
            ...
            while(not trainLoader.finished()):
                ...
                # backward the critic using MSE
                critic_loss = (((baselines - Variable(expanded_reward)) ** 2 ) * weight_variable).sum()
                
                total_critic_loss[sampledSet] += critic_loss.data
                report_critic_loss[sampledSet] += critic_loss.data
                ...
                total_tgt_words[sampledSet] += num_words_sampled
                ...
                if i == 0 or (i % opt.log_interval == -1 % opt.log_interval):
                    ...
                    avgCriticLoss = sum(total_critic_loss.values()) / sum(total_tgt_words.values())
                    logOut = ("Epoch %2d, %5d/%5d; ; %3.0f src tok/s; %3.0f tgt tok/s; avg reward: %6.2f; avg critic loss: %6.2f; lr: %.6f; %s elapsed" %
                                    (epoch, i+1, nSamples,
